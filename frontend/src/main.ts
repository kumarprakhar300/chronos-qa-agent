...
const deviceViewportSelect = document.getElementById('device-viewport') as HTMLSelectElement;
const exportScriptBtn = document.getElementById('export-script') as HTMLButtonElement;
...
runAgentBtn.addEventListener('click', () => {
  ...
  ws.send(JSON.stringify({
    ...
    payload: {
      ...
      apiKey: key,
      headed,
      viewport: deviceViewportSelect.value
    }
  }));
});
...
// Export Playwright TypeScript code
exportScriptBtn.addEventListener('click', () => {
  if (instructions.length === 0) {
    alert('Please add some test instructions first!');
    return;
  }

  // Generate Playwright spec.ts content
  let scriptContent = `import { test, expect } from '@playwright/test';\n\ntest('Chronos Autogenerated Self-Healing Test', async ({ page }) => {\n`;
  
  instructions.forEach((step, idx) => {
    const lowercaseStep = step.toLowerCase();
    scriptContent += `  // Step ${idx + 1}: ${step}\n`;
    if (lowercaseStep.startsWith('go to ')) {
      const url = step.substring(6).trim();
      scriptContent += `  await page.goto('${url}');\n  await page.waitForLoadState('networkidle');\n\n`;
    } else if (lowercaseStep.startsWith('type ') && lowercaseStep.includes(' in the ')) {
      const quoteMatch = step.match(/"([^"]+)"/);
      const textToType = quoteMatch ? quoteMatch[1] : 'text';
      scriptContent += `  // Note: Chronos VLM resolved coordinates and keyboard-typed target element.\n  await page.keyboard.type('${textToType}');\n  await page.keyboard.press('Enter');\n\n`;
    } else if (lowercaseStep.startsWith('click ')) {
      scriptContent += `  // Note: Chronos VLM clicked target element via AI vision-based coordinate matching.\n  // await page.mouse.click(x, y);\n\n`;
    } else {
      scriptContent += `  // Custom instruction: Playwright equivalents will execute via VLM model agent.\n\n`;
    }
  });

  scriptContent += `  console.log('Test execution completed successfully!');\n});\n`;
  ...
  a.href = url;
  a.download = 'chronos-test.spec.ts';
